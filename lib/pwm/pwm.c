...
void PWM_Init(void)
{
  ...
  u16 duty_cycle = eeprom_read_data16(PWM_CYCLE_ADD);
  u16 pwm1_period = eeprom_read_data16(PWM_PERIOD_ADD);
  ...
  // input clk is 2Mhz -> period = 52 means the pwm freq is 2Mhz / 52 ~= 38.5Khz
    ...
    eeprom_write_data16(PWM_CYCLE_ADD, 50);
  ...
  TIM1_OC2Init(TIM1_OCMODE_PWM2,
               ...
               (pwm1_period * duty_cycle / 100) - 1, TIM1_OCPOLARITY_LOW, //脉冲宽度值
